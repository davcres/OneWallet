...
import com.davidcrespo.onewallet.core.generated.resources.rich_phrases
import com.davidcrespo.onewallet.core.generated.resources.total_balance
...
fun OWBalance(
    ...
    val richPhrases = stringArrayResource(Res.array.rich_phrases).toList()
    ...
    val balanceDescription = stringResource(Res.string.total_balance) + ": " + balance.formatTwoDecimals() + " " + currency.symbol
    val trendDescription = if (isExpanded) {
        val changeDescription = when {
            percentage > 0.0 -> stringResource(Res.string.accessibility_percent_increased_by, kotlin.math.abs(percentage).formatTwoDecimals())
            percentage < 0.0 -> stringResource(Res.string.accessibility_percent_decreased_by, kotlin.math.abs(percentage).formatTwoDecimals())
            else -> stringResource(Res.string.accessibility_unchanged)
        }
        val diffDescription = when {
            variance > 0.0 -> stringResource(Res.string.accessibility_price_increased_by, kotlin.math.abs(variance).formatTwoDecimals(), currency.symbol)
            variance < 0.0 -> stringResource(Res.string.accessibility_price_decreased_by, kotlin.math.abs(variance).formatTwoDecimals(), currency.symbol)
            else -> ""
        }
                ...
                AnimatedVisibility(
                    ...
                    Column {
                        Row(
                            ...
                            Text(
                                text = "($prefix${percentage.formatTwoDecimals()} %)",
