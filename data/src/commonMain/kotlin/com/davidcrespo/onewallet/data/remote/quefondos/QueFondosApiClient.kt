...
import com.davidcrespo.onewallet.data.remote.util.normalizeDouble
import com.davidcrespo.onewallet.domain.extensions.round
...
class QueFondosApiClient(private val client: HttpClient) {
    ...
    private fun extractPercent(html: String, label: String): String? {
        return Regex(
            pattern = """(?s)<span[^>]*class="[^"]*floatleft[^"]*"[^>]*>.*?$label\s*</span>\s*<span[^>]*class="[^"]*floatright[^"]*"[^>]*>.*?([\d.,]+)%""",
