...
import com.davidcrespo.onewallet.core.generated.resources.global_markets_request_limit
...
class GlobalMarketViewModelTest {
    ...
    fun `cuando falla la busqueda por query, se muestra el error de limite de peticiones`() = runTest(mainDispatcherExtension.testDispatcher) {
        ...
        viewModel.uiState.test {
            val state = awaitItem()
            assertFalse(state.isLoading)
            assertEquals(Res.string.global_markets_request_limit, state.error)
